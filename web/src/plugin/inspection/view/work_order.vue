    ...
    <div class="scroll-content">
      <el-table :data="tableData" border stripe v-loading="loading" size="small">
        ...
        <el-table-column label="类别" width="80">
          <template #default="s1">{{ catLabel(s1.row.instrumentCategory) }}</template>
        </el-table-column>
        ...
        <el-table-column label="合格/不合格" width="100">
          <template #default="s3">{{ s3.row.passCount || 0 }} / {{ s3.row.failCount || 0 }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="s6">
            <el-button v-if="activeTab === 'pending'" size="small" type="primary" @click="onStartInspect(s6.row)">开始检测</el-button>
            <el-button v-else size="small" type="primary" @click="openDetail(s6.row)">
...
import { startInspection } from '@/plugin/inspection/api/work_order'
...
const onTabChange = () => { sessionStorage.setItem('inspectTab', activeTab.value); searchInfo.page = 1; getList() }

const openDetail = (row) => {
  window.location.hash = `/inspectDetail?orderId=${row.ID}`
}
const onStartInspect = async (row) => {
  await ElMessageBox.confirm('确定开始检测？', '提示', { type: 'info' })
  const res = await startInspection({ ID: row.ID })
  if (res.code === 0) { ElMessage.success('已开始检测'); getList(); openDetail(row) }
}
getList()
</script>

<style scoped>
.list-page { height: 100vh; display: flex; flex-direction: column; background: var(--el-bg-color, #fff); overflow: hidden; }
.sticky-header { flex-shrink: 0; padding: 8px 16px 0 16px; background: var(--el-bg-color, #fff); z-index: 10; box-shadow: 0 2px 8px rgba(0,0,0,0.08); }
.sticky-header :deep(.el-tabs__header) { margin-bottom: 0; }
.search-bar { padding: 4px 0 8px; background: var(--el-bg-color, #fff); }
.scroll-content { flex: 1; overflow-y: auto; padding: 0 16px 16px; }
.pagination-wrap { display: flex; justify-content: flex-end; flex-wrap: wrap; padding: 12px 0 8px; }
