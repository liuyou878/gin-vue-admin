    ...
    <div class="gva-search-box">
      <el-form :model="searchInfo" inline>
        <el-form-item label="生产号">
          <el-input v-model="searchInfo.moNumber" placeholder="请输入MO号" clearable size="small" />
        </el-form-item>
        <el-form-item label="批次号">
          <el-input v-model="searchInfo.batchNumber" placeholder="请输入批次号" clearable size="small" />
        </el-form-item>
        <el-form-item label="SN">
          <el-input v-model="searchInfo.sn" placeholder="请输入SN" clearable size="small" />
        </el-form-item>
        <el-form-item label="型号">
          <el-input v-model="searchInfo.model" placeholder="请输入型号" clearable size="small" />
        </el-form-item>
        <el-form-item>
    ...
    <el-drawer v-model="detailVisible" title="生产工具提交详情" size="60%" destroy-on-close>
      <template v-if="detailData">
        ...
        <el-descriptions :column="2" border size="small" class="mb-4">
          ...
          <el-descriptions-item label="批次号">{{ detailData.batch?.batchNumber || '-' }}</el-descriptions-item>
          <el-descriptions-item label="SN">{{ detailData.sn || '-' }}</el-descriptions-item>
          <el-descriptions-item label="型号">{{ detailData.model || '-' }}</el-descriptions-item>
          <el-descriptions-item label="PN码">{{ detailData.pnCode || '-' }}</el-descriptions-item>
...
const handleDelete = async (row, closeDetail = false) => {
  const label = [row.moNumber || row.productionOrder?.moNumber, row.batchNumber || row.batch?.batchNumber, row.sn]
    .filter(Boolean)
    .join(' / ')
  try {
