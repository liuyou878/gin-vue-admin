    ...
    <template v-else>
      ...
      <div ref="detailScrollRef" class="detail-scroll">
          ...
          <div class="subject-main-row">
            <div class="subject-summary">
              <div class="subject-copy">
                ...
                <el-tag
                  :type="currentSubjectProgressTag"
                  size="large"
                  class="subject-progress"
                >
                  <template v-if="inspectMode === 'byDevice'">
                    <span class="progress-pass"
                      >通过 {{ currentDeviceResultSummary.pass }}</span
                    >
                    <span class="progress-separator">/</span>
                    <span class="progress-fail"
                      >未通过 {{ currentDeviceResultSummary.fail }}</span
                    >
                  </template>
                  <template v-else>
                    {{ currentSubjectProgressText }}
                  </template>
                </el-tag>
              </div>
            </div>
            ...
            >
              <el-option
                ...
                :key="item.itemID"
                :label="`${index + 1}. ${item.itemName} (${doneCount(index)}/${
        ...
        <section class="inspection-list">
          ...
          >
            ...
            <div class="row-controls">
              <div
                v-if="row.result.resultType !== 'number'"
                class="pass-actions"
              >
                <button
        ...
        <div class="detail-footer">
          ...
          <div class="footer-subject-actions">
            <el-button
              size="large"
          ...
          <div class="footer-complete-actions">
            <div class="batch-result-summary">
              <span class="summary-unselected"
                >未选择 {{ batchResultSummary.unselected }}</span
              >
              <span class="summary-pass">通过 {{ batchResultSummary.pass }}</span>
              <span class="summary-fail"
                >未通过 {{ batchResultSummary.fail }}</span
              >
            </div>
            <el-button
              v-if="detail.order.status === 2 && !hasRecheckingDevices"
              type="success"
              size="large"
              @click="onComplete"
              >完成检测</el-button
            >
            <el-button
              v-if="hasRecheckingDevices"
              type="success"
              size="large"
              @click="onCompleteRecheck"
              >完成复检</el-button
            >
          </div>
        </div>
...
<script setup>
  ...
  const recheckEditMode = ref(false)
  ...
  const isRecheckingDevice = (device) => {
    return (
      device?._startedRecheck ||
      (device?._status || device?.status) === 'rechecking'
    )
  }

  const visibleDevices = computed(() => {
    if (!hasRecheckingDevices.value) return detail.value.devices
    return detail.value.devices.filter(isRecheckingDevice)
  })

  const currentDevice = computed(
    () => visibleDevices.value[currentDeviceIndex.value] || null
  )
  const currentItem = computed(
    () => detail.value.templateItems[currentItemIndex.value] || null
  )
  const hasRecheckingDevices = computed(() =>
    detail.value.devices.some(
      (device) =>
        device._startedRecheck ||
        (device._status || device.status) === 'rechecking'
    )
  ...
  const hasPersistedAbnormalDevices = computed(() =>
    detail.value.devices.some((device) =>
      ['fail', 'returned', 'rework', 'pending_recheck', 'rechecking'].includes(
        device.status
      )
    )
  )
  const showResultOverview = computed(
    () =>
      ...
      !recheckEditMode.value
  )
  const isReadonly = computed(
    () => detail.value.order.status === 4 && !recheckEditMode.value
  )

  const detailInfo = computed(() => {
    const order = detail.value.order
    return (
      [order.moNumber, order.batchNumber, order.model, order.inspectorName]
        .filter(Boolean)
        .join(' | ') || '-'
    )
  })
  ...
  const currentSubjectIndex = computed(() =>
    inspectMode.value === 'byItem'
      ? currentItemIndex.value
      : currentDeviceIndex.value
  )
  const currentSubjectTotal = computed(() =>
    inspectMode.value === 'byItem'
  ...
  const currentSubjectSubtitle = computed(() => {
    ...
    if (!currentDevice.value) return '请选择设备'
    if (isRecheckingDevice(currentDevice.value)) {
      return `复检当前设备 ${detail.value.templateItems.length} 个检测项`
    }
    return `按当前设备完成 ${detail.value.templateItems.length} 个检测项`
  })
  ...
  const currentSubjectTotalRows = computed(() => {
    if (inspectMode.value === 'byItem') return visibleDevices.value.length
    return detail.value.templateItems.length
  })
  const currentDeviceResultSummary = computed(() =>
    summarizeDeviceResults(currentDevice.value)
  )
  const batchResultSummary = computed(() =>
    summarizeDevicesResults(
      recheckEditMode.value ? visibleDevices.value : detail.value.devices
    )
  )
  const currentSubjectProgressText = computed(() => {
    if (inspectMode.value === 'byItem') {
      return `${currentSubjectDone.value} / ${currentSubjectTotalRows.value}`
    }
    return `通过 ${currentDeviceResultSummary.value.pass} / 未通过 ${currentDeviceResultSummary.value.fail}`
  })
  const currentSubjectProgressTag = computed(() => {
    if (inspectMode.value === 'byItem') {
      return currentSubjectDone.value === currentSubjectTotalRows.value
        ? 'success'
        : 'warning'
    }
    return currentDeviceResultSummary.value.fail > 0 ? 'warning' : 'success'
  })
  ...
  const inspectionRows = computed(() => {
    ...
    if (!device) return []
    return detail.value.templateItems
      .map((item, itemIndex) => ({
  ...
  const prevSubject = () => {
    if (inspectMode.value === 'byItem' && currentItemIndex.value > 0)
      currentItemIndex.value -= 1
    if (inspectMode.value === 'byDevice' && currentDeviceIndex.value > 0)
  ...
  const nextSubject = () => {
    if (
      inspectMode.value === 'byItem' &&
      currentItemIndex.value < detail.value.templateItems.length - 1
    )
      currentItemIndex.value += 1
    if (
      inspectMode.value === 'byDevice' &&
      currentDeviceIndex.value < visibleDevices.value.length - 1
    )
      currentDeviceIndex.value += 1
  }
  ...
  const summarizeDeviceResults = (device) => {
    ...
    ;(device?.results || []).forEach((result) => {
      if (!resultCompletedForStatus(result)) return
      if (resultFailedForStatus(result)) {
        summary.fail += 1
        return
      }
      summary.pass += 1
  ...
  const summarizeDevicesResults = (devices) => {
    const summary = { unselected: 0, pass: 0, fail: 0 }
  ...
  const resultCompletedForStatus = (result) => {
    ...
    return (
      (result._checked === true || result._checked === false) &&
      result._numVal !== undefined &&
      result._numVal !== null &&
      result._numVal !== ''
    )
  ...
  const resultFailedForStatus = (result) => {
    if (!resultCompletedForStatus(result)) return false
    const passFailed = result._checked === false
    const numberFailed =
      result._numVal !== undefined &&
      result._numVal !== null &&
      result._numVal !== '' &&
      ((result.minValue != null && Number(result._numVal) < result.minValue) ||
        (result.maxValue != null && Number(result._numVal) > result.maxValue))
    return passFailed || numberFailed
  }

  const resultStatusLabel = (result) => {
    if (!resultCompletedForStatus(result)) return '未完成'
    if (resultFailedForStatus(result)) return '未通过'
  ...
  const resultStatusTag = (result) => {
    ...
    if (resultFailedForStatus(result)) return 'danger'
  ...
  const getRangeClass = (result) => {
    ...
    if (
      (min != null && result._numVal < min) ||
      (max != null && result._numVal > max)
    )
      return 'out-range'
  ...
  const resultSaveLabel = (result) => {
    ...
    if (result.inspectorName || result.inspectedAt) {
      return `${result.inspectorName || '已保存'}${
        result.inspectedAt ? ` · ${formatShortTime(result.inspectedAt)}` : ''
      }`
    }
    return '未保存'
  }
  ...
  const resultWasSaved = (result) => {
    if (!result) return false
    return (
      Boolean(result.inspectorName || result.inspectedAt) ||
  ...
  const saveSingleResult = async (device, resultIndex) => {
    ...
    if (recheckEditMode.value && !isRecheckingDevice(device)) return false
    const result = device?.results?.[resultIndex]
    if (!result) return false
    ...
    try {
      const numberResult =
        ...
        result._numVal !== null &&
        result._numVal !== ''
          ? Number(result._numVal)
          : null
      const status =
        detail.value.order.status === 4 &&
        (device._startedRecheck || device.status === 'rechecking')
          ? ''
  ...
  const calcDeviceStatus = (device) => {
    if (device._status === 'rework' || device._status === 'pending_recheck')
      return
    let failed = false
    let passed = false
  ...
  const deviceRowClass = (device) => {
    ...
    return status === 'fail'
      ...
      : status === 'returned' || status === 'rework'
      ? 'row-rework'
      : status === 'pending_recheck' || status === 'rechecking'
      ? 'row-recheck'
      : ''
  }
  ...
  const buildSavePayload = () => {
    ...
    const deviceResults = []
    const devices = hasRecheckingDevices.value
  ...
  const shouldValidateDevice = (device) => {
    if (detail.value.order.status === 2)
      return (device._status || device.status) !== 'rework'
  ...
  const getIncompleteSummary = () => {
    ...
    detail.value.devices.forEach((device) => {
      ...
      device.results.forEach((result) => {
        if (!resultCompletedForStatus(result))
          missing.push(`${device.sn} / ${result.itemName}`)
      })
    })
    return missing
  }
  ...
  const onComplete = async () => {
    ...
    const completeMessage =
      batchResultSummary.value.fail > 0
        ? `确定完成该工单的检测？\n\n当前未通过 ${batchResultSummary.value.fail} 项，提交后可以处理不合格设备打回生产。`
        : '确定完成该工单的检测？\n\n当前全部通过，提交完成后检测结果将不可更改，请确认。'
    await ElMessageBox.confirm(completeMessage, '提示', { type: 'info' })
  ...
  const onCompleteRecheck = async () => {
    ...
    if (res.code === 0) {
      ...
      recheckEditMode.value = false
      await loadDetail()
    }
  }

  const handleGoRecheck = async ({ deviceID } = {}) => {
    recheckEditMode.value = true
    pendingFocusDeviceID.value = deviceID || null
    await focusPendingDevice()
  }

  const focusPendingDevice = async () => {
    if (!pendingFocusDeviceID.value) return
    const focusIndex = visibleDevices.value.findIndex(
      (device) => device.ID === pendingFocusDeviceID.value
    )
    if (focusIndex >= 0) {
      inspectMode.value = 'byDevice'
      currentDeviceIndex.value = focusIndex
      currentItemIndex.value = 0
      scrollDetailToTop()
    }
    pendingFocusDeviceID.value = null
  }
  ...
  const loadDetail = async () => {
    ...
    data.devices.forEach((device) => {
      ...
      if (
        ...
        device._status !== 'pending_recheck' &&
        device._status !== 'rechecking'
      ) {
        calcDeviceStatus(device)
      }
    ...
    detail.value = data
    await focusPendingDevice()
    if (!hasRecheckingDevices.value) recheckEditMode.value = false
    if (currentItemIndex.value >= detail.value.templateItems.length)
      currentItemIndex.value = 0
    if (currentDeviceIndex.value >= visibleDevices.value.length)
...
<style scoped>
  ...
  .subject-progress {
    --el-tag-text-color: var(--el-text-color-regular, #606266);
  }
