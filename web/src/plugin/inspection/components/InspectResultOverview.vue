  ...
  <div class="result-overview">
    <section class="overview-header">
      <div>
        ...
        <div class="overview-subtitle">
          {{ detail.order.moNumber || '-' }} /
          {{ detail.order.batchNumber || '-' }}
        </div>
      </div>
    ...
    <section class="info-grid">
      <div class="info-card">
        <span>模板</span>
        <strong>{{
          detail.order.templateName || detail.order.productName || '-'
        }}</strong>
      </div>
      <div class="info-card">
        <span>型号</span>
    ...
    <section v-if="canReturn && failDevices.length" class="return-panel">
      ...
      <el-button
        type="warning"
        :disabled="returnDeviceIDs.length === 0"
        @click="onReturnDevices"
      >
        打回生产
      </el-button>
    </section>
    ...
    >
      ...
      <el-table-column label="完成项" width="90">
        <template #default="scope"
          >{{ scope.row.doneCount }} / {{ scope.row.totalCount }}</template
        >
      </el-table-column>
      <el-table-column label="失败项" width="90">
        <template #default="scope">
          <span :class="{ 'fail-text': scope.row.failCount > 0 }">{{
            scope.row.failCount
          }}</span>
        </template>
      </el-table-column>
...
<script setup>
  ...
  const hasRecheckDevices = computed(() =>
    normalizedDevices.value.some((device) =>
      ['pending_recheck', 'rechecking'].includes(
  ...
  const canReturn = computed(
    () => props.detail.order?.status === 2 && !hasRecheckDevices.value
  )

  const failDevices = computed(() =>
    normalizedDevices.value.filter(
      (device) => (device.status || device._status) === 'fail'
    )
  )
...
<style scoped>
  ...
  .return-panel {
    ...
    grid-template-columns:
      minmax(180px, 260px) minmax(220px, 1fr) minmax(220px, 1fr)
      auto;
    align-items: center;
    gap: 10px;
    margin-bottom: 14px;
